...
#include <iostream>
#include <fstream>          // 读取文件：vert和frag
#include <stdexcept>		// 异常处理框架
#include <algorithm>
#include <vector>
#include <cstring>
#include <functional>		// 资源管理
#include <cstdlib>			// 用来包含EXIT_SUCCESS和EXIT_FAILURE
#include <cstdint>
#include <set>       
...
private:
	...
	VkExtent2D chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
		if (capabilities.currentExtent.width != UINT32_MAX) {
			...
			actualExtent.width = max(capabilities.minImageExtent.width, std::min(capabilities.maxImageExtent.width, actualExtent.width));
			actualExtent.height = max(capabilities.minImageExtent.height, std::min(capabilities.maxImageExtent.height, actualExtent.height));
			return actualExtent;
		}
	}
